...
export async function createAgent(): Promise<Agent> {
  ...
  // Initialize with CDP wallet provider (Base Mini App wallet context available on client-side)
  const cdpResult = await prepareAgentkitAndWalletProvider();
  const agentkit = cdpResult.agentkit;
  const walletProvider = cdpResult.walletProvider;
